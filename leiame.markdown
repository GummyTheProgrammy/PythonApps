...
- **VideoTools** (VideoTools/): utilitários para processamento de vídeo em lote.
**SmartVideoCutter**: concatenar multiplos vídeos numa timeline, depois os corta em partes regulares, permitindo ajuste de largura e altura. **TimelineTotalizer**:  calcula o tempo total de duração de todos os vídeos em uma pasta e subpastas. **MultiVideoInspector**: Este script lista a resolução (largura x altura) de todos os vídeos na pasta selecionada. **MultiVideoToFrames**: extrai todos os frames de vídeos em uma pasta e os salva como imagens PNG."
