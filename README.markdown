...
- **VideoTools**: utilities for batch video processing. **SmartVideoCutter**: concatenates multiple videos into a timeline, then cuts them into regular parts, allowing width and height adjustment. **TimelineTotalizer**: calculates the total duration of all videos in a folder and subfolders. **MultiVideoInspector**: This script lists the resolution (width x height) of all videos in the selected folder. **MultiVideoToFrames**: extracts all video frames in a folder and saves them as PNG images.
